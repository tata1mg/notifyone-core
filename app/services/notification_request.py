...
class NotificationRequest:
    ...
    async def _handle_notification_request(cls, request_body, message_receive_count=1):
        print('Handling notification request: {}'.format(request_body))
        ...
        logger.info('Transforming request body for event: {}'.format(event_name))
        ...
        if message_receive_count > 1:
            ...
            asyncio.create_task(EventManager.save_event_body(str(event.id), request_body.get('body')))
        ...
        # Get eligible channels
        active_channels = event.get_allowed_channels(request_body.get('channels'))
        eligible_channels = set(active_channels).difference(barred_channels)

        tasks = {}
        # Dispatch notification request to respective handlers
        print(f"Eligible channels for event {event_name}: {eligible_channels}")
        print(f"Request body: {request_body}")
        # return await EmailHandler.handle(event, request_body, log_details[NotificationChannels.EMAIL.value])
        ...
        if event.is_trigger_limit_enabled_for_any_active_channel():
            ...
            for channel in active_channels:
                if event.is_trigger_limit_enabled_for_channel(channel) and event.triggers_limit[channel] < triggered_counts[channel]:
                    logger.info('Trigger limit breached. Event - {}, Channel - {}, Identifier - {}'.format(event.event_name, channel, NotificationChannels.get_source_identifier_for_event(event.event_name, request_body)))
